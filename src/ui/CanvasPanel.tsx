...
export default function CanvasPanel({ artifact, generating }: Props) {
  ...
  const exportTweaks = artifact ? tweaks[artifact.id] : undefined;
  ...
  const previewHtml = useMemo(() => {
    ...
    const tweakCss = t ? buildTweakInjection(t) : "";
...
${js ? `<script>${js}</script>` : ""}${PREVIEW_SNIPPET}`;
  ...
  const openInBrowser = () => {
    const exportResult = buildHtmlExport(artifact, exportTweaks);
